...
regresults <- read.csv("WRegularSeasonDetailedResults.csv")
results <- read.csv("WNCAATourneyDetailedResults.csv")
...
r1 = regresults[, c("Season", "DayNum", "WTeamID", "WScore", "LTeamID", "LScore", "NumOT",
                    "WFGA", "WAst", "WBlk", "LFGA", "LAst", "LBlk")]
r2 = regresults[, c("Season", "DayNum", "LTeamID", "LScore", "WTeamID", "WScore", "NumOT",
                    "LFGA", "LAst", "LBlk", "WFGA", "WAst", "WBlk")]
names(r1) = c("Season", "DayNum", "T1", "T1_Points", "T2", "T2_Points", "NumOT", "T1_fga",
              "T1_ast", "T1_blk", "T2_fga", "T2_ast", "T2_blk")
names(r2) = c("Season", "DayNum", "T1", "T1_Points", "T2", "T2_Points", "NumOT", "T1_fga", 
              "T1_ast", "T1_blk", "T2_fga", "T2_ast", "T2_blk")
...
W$Pred = Reduce("+", probs) / 10


W$Pred[Z$Pred <= 0.025] = 0.025
W$Pred[Z$Pred >= 0.975] = 0.975

#overrides
W$Pred[W$Seed1 >= 8 & W$Seed2 == 1] = 0
W$Pred[W$Seed1 >= 7 & W$Seed2 == 2] = 0
W$Pred[W$Seed1 == 14 & W$Seed2 == 3] = 0
W$Pred[W$Seed1 == 13 & W$Seed2 == 4] = 0
W$Pred[W$Seed1 == 1 & W$Seed2 >= 8] = 1
W$Pred[W$Seed1 == 2 & W$Seed2 >= 7] = 1
W$Pred[W$Seed1 == 3 & W$Seed2 == 14] = 1
W$Pred[W$Seed1 == 4 & W$Seed2 == 13] = 1

left_side <- c(3124, 3101, 3143, 3314, 3199, 3137, 3376, 3125,
               3246, 3343, 3301, 3263, 3281, 3179, 3234, 3273,
               3280, 3380, 3377, 3155, 3113, 3416, 3274, 3195,
               3355, 3346, 3393, 3200, 3400, 3231, 3332, 3340)
W <- W %>% mutate(t1_left_side = ifelse(team1 %in% left_side, 1, 0))
W <- W %>% mutate(t2_left_side = ifelse(team2 %in% left_side, 1, 0))
W <- W %>% mutate(only_finals = ifelse(t1_left_side + t2_left_side == 1, 1, 0))
W$Pred[Z$only_finals == 1] = 1


write.csv(select(W, ID, Pred), "sub1.csv", row.names=FALSE)
